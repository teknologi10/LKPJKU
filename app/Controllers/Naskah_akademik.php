...
class Naskah_akademik extends BaseController
{
    ...
    public function index()
    {
        ...
        $data = [
            ...
            'terbaru' => $this->naskahModel->orderBy('id_naskah_akademik', 'desc')->paginate(3),
    ...
    public function update($id)
    {
        ...
        $filenaskah = $this->request->getFile('naskah');

        //cek gambar, apakah tetap gambar lama
        if ($filenaskah == '') {
            $namanaskah = $this->request->getVar('naskahlama');
        } else {
            //pindah gambar
            $filenaskah->move('peraturan/naskah_akademik');

            //generate nama file
            $namanaskah = $fileRaperda->getName();
